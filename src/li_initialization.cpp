...
void LI_Init_set()
{
    ...
    if (use_imu_as_input)
    {
        kf_input.x_.pos = -(p_imu->state_LI_Init.rot * Lidar_R_wrt_IMU.transpose() * Lidar_T_wrt_IMU + // .normalized().toRotationMatrix()
                        p_imu->state_LI_Init.pos); //Body frame is IMU frame in Point-LIO mode
        kf_input.x_.rot = p_imu->state_LI_Init.rot * Lidar_R_wrt_IMU.transpose(); // .normalized().toRotationMatrix()
    ...
    {
        kf_output.x_.pos = -1 * (p_imu->state_LI_Init.rot * Lidar_R_wrt_IMU.transpose() * Lidar_T_wrt_IMU + // .normalized().toRotationMatrix()
                        p_imu->state_LI_Init.pos); //Body frame is IMU frame in Point-LIO mode
        kf_output.x_.rot = p_imu->state_LI_Init.rot * Lidar_R_wrt_IMU.transpose(); // .normalized().toRotationMatrix()
        ...
        kf_output.x_.acc = - kf_output.x_.rot.transpose() * gravity_lio; // .normalized().toRotationMatrix()
    }
    {
        // init_map = true;
