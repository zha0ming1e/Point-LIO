...
class GnssPsrDoppFactorNolidar : public gtsam::NoiseModelFactor4<gtsam::Rot3, gtsam::Vector12, gtsam::Vector4, gtsam::Vector1>
{
    public: 
        ...
        GnssPsrDoppFactorNolidar(gtsam::Key j1, gtsam::Key j2, gtsam::Key j3, gtsam::Key j4, double values_[30], int sys_idx_, Eigen::Vector3d hat_omg_T_, const gtsam::SharedNoiseModel& model) :
        ...
        {          
            ...
            {    
                ...
                if (H1)
                {
                    ...
                    (*H1).block<1,3>(0,0) = rcv2sat_unit.transpose() * hat_T * pr_weight; 
                    (*H1).block<1,3>(1,0) = -(sv_vel-V_ecef).transpose() * unit2rcv_pos * hat_T * dp_weight;
                }
