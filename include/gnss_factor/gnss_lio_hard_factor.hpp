...
{
    public: 
        GnssLioHardFactor(gtsam::Key j1, gtsam::Key j2, Eigen::Vector3d pos_lio_, Eigen::Vector3d vel_lio_, Eigen::Matrix3d rot_lio_, Eigen::Matrix<double, 9, 9>  sqrt_lidar_, const gtsam::SharedNoiseModel& model) : 
        rot_lio(rot_lio_), pos_lio(pos_lio_), vel_lio(vel_lio_), sqrt_lidar(sqrt_lidar_), gtsam::NoiseModelFactor2<gtsam::Rot3, gtsam::Vector6>(model, j1, j2) {}
